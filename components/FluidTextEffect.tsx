...
function extractFirstColor(input: string) {
  if (!input) return null;
  const rgbMatch = input.match(/rgba?\([^)]+\)/i);
  if (rgbMatch) {
    const rgb = parseRgbString(rgbMatch[0]);
...
function resolveColorFromClass(colorClass?: string) {
  if (!colorClass) return null;
  if (COLOR_CACHE.has(colorClass)) return COLOR_CACHE.get(colorClass) || null;

  const el = document.createElement('div');
  el.className = `${colorClass} hidden`;
  ...
  COLOR_CACHE.set(colorClass, hex);
  return hex;
}

function resolveBaseColor(colorClass?: string, customBackground?: string) {
  if (customBackground) {
    const extracted = extractFirstColor(customBackground);
    if (extracted) return extracted;
  }

  if (colorClass) {
    const base = BASE_COLORS.find((c) => c.bg === colorClass);
    if (base) {
      const extracted = extractFirstColor(base.hex);
      if (extracted) return extracted;
    }
    const resolved = resolveColorFromClass(colorClass);
...
function getHexCSSVar(name: string) {
  const raw = getComputedStyle(document.documentElement).getPropertyValue(name).trim();
  if (!raw) return '#000000';
  const hex = extractFirstColor(raw);
  if (hex) return hex;
...
const FluidTextEffect: React.FC<FluidTextEffectProps> = ({
  text,
  ...
  const scheduleMaskDrawRef = useRef<() => void>(() => {});
  const isInitializedRef = useRef(false);
  const textRef = useRef<string>(text?.trim() || 'Text');
  const fontSizeRef = useRef<number>(
    Number.isFinite(fontSize) ? clamp01(fontSize) : DEFAULT_FONT_SIZE
  );

  const resolvedText = text?.trim() || 'Text';
  const resolvedFontSize = Number.isFinite(fontSize) ? clamp01(fontSize) : DEFAULT_FONT_SIZE;

  const isTransparent = useMemo(() => {
    if (colorClass === 'bg-transparent') return true;
    if (customBackground) return customBackground.toLowerCase().includes('transparent');
    return false;
  }, [colorClass, customBackground]);
  ...
  useEffect(() => {
    ...
    if (!container || !fluidCanvas || !maskCanvas || !shadowCanvas) return;

    let animationId = 0;
    let splatIntervalId: ReturnType<typeof setInterval> | null = null;
    let maskDrawRaf = 0;
    let maskReady = false;
    ...
    function drawOverlayCanvas() {
      ...
      if (shadowCanvas && isTransparent) {
        ...
        const textFontSize = Math.round(width * fontSizeRef.current);
        shadowCtx.font = `${fontWeight} ${textFontSize}px ${fontFamily}`;
        shadowCtx.textAlign = 'center';

        const metrics = shadowCtx.measureText(textRef.current);
        let textY = height / 2;
        if (
        ...
        shadowCtx.fillText(textRef.current, width / 2, textY);
      ...
      const metrics = ctx.measureText(textRef.current);
      let textY = height / 2;
      if (
        metrics.actualBoundingBoxAscent !== undefined &&
        metrics.actualBoundingBoxDescent !== undefined
      ...
      if (isTransparent) {
        ...
        ctx.strokeText(textRef.current, width / 2, textY);
        ctx.globalAlpha = 1;
        ctx.globalCompositeOperation = 'source-over';
      ...
      ctx.globalCompositeOperation = 'destination-out';
      ctx.fillText(textRef.current, width / 2, textY);
      ctx.globalCompositeOperation = 'source-over';
      maskReady = true;
    }
    ...
    scheduleMaskDrawRef.current = scheduleMaskDraw;

    function initFluidSimulation(startHue: number, endHue: number) {
      if (!fluidCanvas || !maskCanvas || !container) return;
      if (fluidCanvas.clientWidth === 0 || fluidCanvas.clientHeight === 0) {
        requestAnimationFrame(() => initFluidSimulation(startHue, endHue));
        return;
      }
      ...
      if (!ext.supportLinearFiltering) {
        ...
        (config as any).BLOOM = false;
        (config as any).SUNRAYS = false;
      }
      ...
      function updateKeywords() {
        ...
        if ((config as any).SHADING) displayKeywords.push('SHADING');
        if ((config as any).SUNRAYS) displayKeywords.push('SUNRAYS');
      ...
      function splatPointer(pointer: Pointer) {
        const dx = pointer.deltaX * (config as any).SPLAT_FORCE * 12;
        const dy = pointer.deltaY * (config as any).SPLAT_FORCE * 12;
      ...
      function step(dt: number) {
        ...
        gl.uniform1f(vorticityProgram.uniforms.curl, (config as any).CURL);
        ...
        gl.uniform1f(clearProgram.uniforms.value, (config as any).PRESSURE);
        blit(pressure.write);
        pressure.swap();
        ...
        for (let i = 0; i < (config as any).PRESSURE_ITERATIONS; i++) {
        ...
        gl.uniform1f(advectionProgram.uniforms.dissipation, (config as any).VELOCITY_DISSIPATION);
        blit(velocity.write);
        velocity.swap();
        ...
        gl.uniform1f(advectionProgram.uniforms.dissipation, (config as any).DENSITY_DISSIPATION);
        blit(dye.write);
        dye.swap();
      }
      ...
      function applySunrays(source: FBO, mask: FBO, destination: FBO) {
        ...
        sunraysProgram.bind();
        gl.uniform1f(sunraysProgram.uniforms.weight, (config as any).SUNRAYS_WEIGHT);
      ...
      function drawDisplay(target: FBO | null) {
        ...
        if ((config as any).SHADING) {
          gl.uniform2f(displayMaterial.uniforms.texelSize, 1.0 / width, 1.0 / height);
        }
        gl.uniform1i(displayMaterial.uniforms.uTexture, dye.read.attach(0));
        if ((config as any).SUNRAYS) {
      ...
      function render(target: FBO | null) {
        ...
        if (target === null || !(config as any).TRANSPARENT) {
        ...
        if (!(config as any).TRANSPARENT) {
          drawColor(target, {
            r: (config as any).BACK_COLOR.r / 255,
            g: (config as any).BACK_COLOR.g / 255,
            b: (config as any).BACK_COLOR.b / 255,
          });
        }
        drawDisplay(target);
      }
      ...
      function updateColors(dt: number) {
        if (!(config as any).COLORFUL) return;
        colorUpdateTimer += dt * (config as any).COLOR_UPDATE_SPEED;
      ...
      function update() {
        const dt = calcDeltaTime() * ((config as any).RENDER_SPEED ?? 1.0);
        ...
        updateColors(dt);
        applyInputs();
        if (!(config as any).PAUSED) step(dt);
        render(null);
        animationId = requestAnimationFrame(update);
      }
      ...
      handleMouseEnter = (e: MouseEvent) => {
        if (fluidCanvas.width === 0 || fluidCanvas.height === 0) return;
      ...
      handleMouseMove = (e: MouseEvent) => {
        ...
        if (pointer.moved) {
          ...
          splat(
            pointer.texcoordX,
            pointer.texcoordY,
            pointer.deltaX * (config as any).SPLAT_FORCE * 12,
            pointer.deltaY * (config as any).SPLAT_FORCE * 12,
            {
              r: pointer.color[0],
              g: pointer.color[1],
              b: pointer.color[2],
      ...
      if (handleMouseEnter) container.addEventListener('mouseenter', handleMouseEnter);
    ...
    const computedColor = resolveBaseColor(colorClass, customBackground);
    ...
    if (document.fonts?.ready) {
      document.fonts.ready.then(() => {
        if (isInitializedRef.current) scheduleMaskDraw();
      });
    }
  ...
  }, [colorClass, customBackground, isTransparent]);

  useEffect(() => {
    textRef.current = resolvedText;
    fontSizeRef.current = resolvedFontSize;
    if (isInitializedRef.current) {
      scheduleMaskDrawRef.current();
    }
  }, [resolvedText, resolvedFontSize, isTransparent]);

  return (
    <div
      ref={containerRef}
      className={`relative h-full w-full overflow-hidden rounded-[inherit] pointer-events-auto ${
        isTransparent ? 'bg-gray-50' : 'bg-black'
      }`}
    >
      <canvas
        ref={shadowCanvasRef}
        className="absolute inset-0.5 h-[calc(100%-4px)] w-[calc(100%-4px)] rounded-[inherit]"
      />
      <canvas
        ref={fluidCanvasRef}
        className="absolute inset-0.5 h-[calc(100%-4px)] w-[calc(100%-4px)]"
      />
      <canvas ref={maskCanvasRef} className="absolute h-full w-full" />
    </div>
  );
};

export default FluidTextEffect;
